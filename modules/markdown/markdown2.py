...
class Markdown(object):
    ...
    def _sanitize_html(self, s):
        if self.safe_mode == "replace":
            ...
            return s
        elif self.safe_mode == "safe":
            return s
        else:
            raise MarkdownError("invalid value for 'safe_mode': %r (must be "
                                "'escape', 'replace' or 'safe')" % self.safe_mode)
            ...
            if text[p] == '(': # attempt at perf improvement
                ...
                if match:
                    ...
                    if title:
                        ...
                        title_str = ''
                    if is_img:
                        result = '<div class="markdownimagecontainer"><img class="markdownimage" src="%s" alt="%s"%s%s' \
                            % (url.replace('"', '&quot;'),
                               link_text.replace('"', '&quot;'),
                               title_str, self.empty_element_suffix)
                        curr_pos = start_idx + len(result)
                        text = text[:start_idx] + result + text[match.end():] + "<br/>" + str(link_text).replace('"', '&quot;') + "</div>"
...
def main(argv=None):
    ...
    parser.add_option("-s", "--safe", metavar="MODE", dest="safe_mode",
                      help="sanitize literal HTML: 'escape' escapes "
                           ...
                           "[HTML_REMOVED] note, and 'safe' leaves it as is")
